...
async function getHeaders(url) {
    ...
    if (customHeaders) {
        if (!(customHeaders instanceof fetch.Headers)) {
            try {
                customHeaders = new fetch.Headers(customHeaders);
            }
            catch (e) {
...
async function countTokens(apiKey, model, params, requestOptions) {
    const response = await makeRequest(model, Task.COUNT_TOKENS, apiKey, false, JSON.stringify(params), requestOptions);
